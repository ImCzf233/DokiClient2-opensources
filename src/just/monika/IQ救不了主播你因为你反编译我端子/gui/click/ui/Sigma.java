...
extends UI {
    ...
    public void categoryButtonDraw(CategoryButton p0, float p2, float p3) {
        ...
        DokiDokiLegitClient.fm.getFont("SFM 12").drawCenteredString(p0.name, p0.x + x, p0.y + y + 2.0f, color);
        GlStateManager.enableBlend();
        switch (p0.name) {
            case "Combat": {
    ...
    public void categoryPanelConstructor(CategoryPanel categoryPanel, CategoryButton categoryButton, float x, float y) {
        ...
        if (categoryButton.name.equalsIgnoreCase("Colors")) {
            categoryPanel.colorPreviews.add(new ColorPreview(DokiDokiLegitClient.cm.getHudColor(), "Hud Color", 10.0f, 0.0f, categoryButton));
        }
    }
    ...
    public void categoryPanelDraw(CategoryPanel categoryPanel, float x, float y) {
        ...
        if (categoryPanel.settingModule != null) {
            DokiDokiLegitClient.fm.getFont("SFR 10").drawString(categoryPanel.settingModule.getName(), x1 - 140.0f, y1 - 133.0f, Colors.getColor(32));
        }
        GL11.glDisable((int)3089);
        GL11.glPopMatrix();
    }
    ...
    public void buttonDraw(Button p0, float p2, float p3, CategoryPanel panel) {
        ...
        DokiDokiLegitClient.fm.getFont("SFR 7").drawString(meme, x1 + 2.0f, y1 + p0.y + 20.0f, Colors.getColor(32));
        if (this.getSettings(p0.module) != null) {
            DokiDokiLegitClient.fm.getFont("SFM 6").drawString("Settings", x1 + 36.0f, y1 + p0.y + 15.0f, Colors.getColor(32));
            GlStateManager.pushMatrix();
        ...
        if (hovered) {
            ...
            this.drawModalRectWithCustomSizedTexture(-5.0f, -5.0f, 0.0f, 0.0f, 10.0f, 10.0f, 10.0f, 10.0f);
            GlStateManager.popMatrix();
            DokiDokiLegitClient.fm.getFont("SFM 8").drawString(p0.module.getDescription(), x1 - p0.x - 15.0f, y1 + 295.0f, Colors.getColor(32));
        }
    }
    ...
    public void rgbSliderDraw(RGBSlider slider, float x, float y) {
        ...
        DokiDokiLegitClient.fm.getFont("SFR 6").drawString(current, xOff - 6.0f, yOff + 2.5f, -1);
        float textX = xOff + 30.0f - DokiDokiLegitClient.fm.getFont("SFR 6").getWidth(Integer.toString((int)value)) / 2.0f;
        DokiDokiLegitClient.fm.getFont("SFR 6").drawString(Integer.toString((int)value), textX, yOff + 5.0f, -1);
        double newValue = 0.0;
        if (slider.dragging) {
